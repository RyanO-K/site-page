...
const PROJECTS_PATH = path_1.default.resolve(__dirname, '../projects.json');
...
function readProjects() {
    try {
        return JSON.parse(fs_1.default.readFileSync(PROJECTS_PATH, 'utf-8'));
    }
    catch {
        return [];
    }
...
function writeProjects(projects) {
    fs_1.default.writeFileSync(PROJECTS_PATH, JSON.stringify(projects, null, 2));
}
...
function serveStatic(res, urlPath) {
    const abs = path_1.default.join(PUBLIC_DIR, urlPath === '/' ? '/index.html' : urlPath);
    const ext = path_1.default.extname(abs);
    fs_1.default.readFile(abs, (err, data) => {
        if (err) {
            res.writeHead(404);
            res.end('Not found');
            return;
...
const server = http_1.default.createServer(async (req, res) => {
    ...
    try {
        ...
        if (method === 'GET' && urlPath === '/api/me') {
            json(res, 200, { user: getSessionUser(req) });
            return;
        }
        if (method === 'GET' && urlPath === '/api/projects') {
            json(res, 200, readProjects());
            return;
        }
        if (method === 'POST' && urlPath === '/api/projects') {
            ...
            if (!repo || !hostedUrl) {
                ...
                json(res, 400, { error: 'repo (owner/name) and url are required' });
                return;
            }
            const ghHeaders = { 'User-Agent': 'portfolio', Accept: 'application/vnd.github+json' };
